...
def _fetch_fr24_flights(session: requests.Session, flight_type: str, timestamp: int, max_pages: int = MAX_PAGES) -> list[dict]:
    rows = []
    
    for page in range(1, max_pages + 1):
        url = (
            f"https://api.flightradar24.com/common/v1/airport.json"
            f"?code=sgn&plugin[]=&plugin-setting[schedule][mode]={flight_type}"
...
def get_tia_flights(flight_type="arrival", max_pages=MAX_PAGES, flight_date: str | None = None):
    ...
    print(f"\n[DANG LAY DU LIEU {flight_direction.upper()} TU FLIGHTRADAR24]")
    
    raw_data = _fetch_fr24_flights(session, fr24_mode, timestamp, max_pages)
...
def main():
    ...
    if df_all.empty:
        print("TSN: Khong thu thap duoc du lieu chuyen bay.")
        return
    ...
    print(f"TSN: Da ghi {inserted}/{len(db_df)} dong vao PostgreSQL.")
